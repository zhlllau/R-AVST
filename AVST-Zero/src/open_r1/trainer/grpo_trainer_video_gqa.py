...
class Qwen2VLGRPOTrainer_Video_GQA(Trainer):
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                ...
                tokenizer = AutoProcessor.from_pretrained(model, padding_side="left")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        video_inputs = inputs[0]["video_inputs"]
        fps_inputs = inputs[0]["video_kwargs"]["fps"]
        image_inputs = None
        ...
        try:
            ...
            if "CUDA out of memory" in str(e):
                self.accelerator.print("CUDA OOM during generation. Skipping this batch.")
        ...
        try:
            ...
            per_token_logps = self._get_per_token_logps(
                model, prompt_completion_ids, attention_mask, pixel_values_videos, video_grid_thw
            )
            ...
            if "CUDA out of memory" in str(e):
                self.accelerator.print("CUDA OOM during logprob computation. Skipping this batch.")
                torch.cuda.empty_cache()
                return torch.tensor(0.0, requires_grad=True, device=device)
            else:
                raise e
            ...
            for i, (reward_func, reward_processing_class) in enumerate(zip(self.reward_funcs, self.reward_processing_classes)):
                try:
                    if isinstance(reward_func, PreTrainedModel):
                        ...
                        required_args_map = {
                            "iou_glue_reward": ["solution", "durations"],
                            "answer_reward": ["solution"],
                            "format_reward": [],
                            "spatial_temporal_reward": ["solution", "durations", "reasoning_type"]
                        }
                        reward_kwargs = {}
                        for arg in required_args_map.get(reward_func_name, []):
                            reward_kwargs[arg] = [ex[arg] for ex in inputs for _ in range(self.num_generations)]
                        rewards_per_func[:, i] = torch.tensor(
                            reward_func(completions=completions, **reward_kwargs),
                            dtype=torch.float32, device=device
                        )
                except RuntimeError as e:
                    self.accelerator.print(f"Reward func '{reward_func_name}' failed with OOM. Setting to 0.")
                    torch.cuda.empty_cache()
                    rewards_per_func[:, i] = 0.0
        except Exception as e:
            self.accelerator.print(f"Unexpected error in reward computation: {str(e)}")
            return torch.tensor(0.0, requires_grad=True, device=device)
