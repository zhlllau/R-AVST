...
def _read_video_torchvision(
    ele: dict, client=None
) -> (torch.Tensor, float):
    # zl: 注释掉了raise NotADirectoryError
...
def fetch_video(ele: dict, image_factor: int = IMAGE_FACTOR, return_video_sample_fps: bool = False, client=None) -> torch.Tensor | list[Image.Image]:
...
def process_vision_info(
    ...
    for vision_info in vision_infos:
        if "image" in vision_info['type'] or "image_url" in vision_info['type']:
            image_inputs.append(fetch_image(vision_info))
        elif "video" in vision_info['type']:
